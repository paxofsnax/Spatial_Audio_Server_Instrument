...
impl<T> MsQueue<T> {
    ...
    pub fn new() -> MsQueue<T> {
        ...
        // Antopia: mem::uninitialized() panics at runtime on rustc >= 1.51 for
        // types that are not uninit-valid (e.g. enums containing Vec/String).
        // The sentinel's payload is never read as a `T` (only its discriminant
        // is matched), so we replicate the pre-1.51 behaviour via MaybeUninit.
        // See also: seg_queue.rs Segment::new().
        let sentinel = Owned::new(Node {
            payload: Payload::Data(unsafe {
