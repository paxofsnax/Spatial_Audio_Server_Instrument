...
impl<T> Segment<T> {
    fn new() -> Segment<T> {
        ...
        // types that are not uninit-valid (e.g. Vec-containing payloads).
        // Each slot's `T` is written via ptr::write before it is read, so we
        // replicate the pre-1.51 behaviour via MaybeUninit.
        // See also: ms_queue.rs MsQueue::new().
        let rqueue = Segment {
