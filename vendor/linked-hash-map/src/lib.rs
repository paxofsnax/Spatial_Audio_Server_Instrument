...
impl<K, V, S> LinkedHashMap<K, V, S> {
    ...
    fn ensure_guard_node(&mut self) {
        if self.head.is_null() {
            ...
            unsafe {
                self.head = // Antopia: mem::uninitialized() panics at runtime on rustc >= 1.51;
                // the guard node's key/value are never read, so replicate the
                // pre-1.51 behaviour via MaybeUninit.
...
impl<K: Hash + Eq, V, S: BuildHasher> IntoIterator for LinkedHashMap<K, V, S> {
    ...
    fn into_iter(mut self) -> IntoIter<K, V> {
        ...
        // Antopia: see above; the uninitialised map is immediately forgotten.
