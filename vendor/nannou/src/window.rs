    ...
    pub fn inner_size_points(&self) -> (f32, f32) {
        ...
        gl_window
            ...
            .map(|(w_px, h_px)| {
                ...
                // Antopia: upstream MULTIPLIED pixels by the hidpi factor here, but
                // pixels are already points * hidpi. On Retina (hidpi=2) this made
                // inner_size_points return 4x the real size, so the conrod UI was
                // laid out for a window 4x larger than the actual one (side menu
                // off-screen, broken pan/zoom) until a resize event fixed it.
                // Points = pixels / hidpi.
